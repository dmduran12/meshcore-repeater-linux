...
class StorageCollector:
    ...
    def get_noise_floor_rrd(self, hours: int = 24) -> dict:
        """
        Get noise floor data formatted as GraphData for charts.
        Returns data in the format expected by the frontend's graph components.
        """
        history = self.sqlite_handler.get_noise_floor_history(hours)
        ...
        timestamps = [int(entry["timestamp"]) for entry in history]
        values = [[int(entry["timestamp"]), entry["noise_floor_dbm"]] for entry in history]
