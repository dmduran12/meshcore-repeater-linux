    ...
    async def send_advert(self) -> bool:
        ...
        try:
            ...
            if self.repeater_handler:
                ...
                # Record the transmitted packet to storage for stats tracking
                pkt_hash = packet.calculate_packet_hash().hex().upper()
                tx_record = {
                    "timestamp": time.time(),
                    "header": f"0x{packet.header:02X}" if packet.header else None,
                    ...
                    "drop_reason": None,
                    "src_hash": f"{self.local_hash:02X}",
                    "dst_hash": None,
                    "path_hash": None,
                    "original_path": None,
                    "forwarded_path": None,
                    "raw_packet": packet.write_to().hex() if hasattr(packet, "write_to") else None,
                    "packet_origin": "tx_local",  # Locally originated transmission
                }
                
                if self.repeater_handler.storage:
                    self.repeater_handler.storage.record_packet(tx_record)
                    self.repeater_handler.tx_count += 1  # Increment local TX count, not forwarded
                    logger.debug("Recorded sent advert to storage")
